...
public sealed class MCH_Default : MCH_Base
{
    ...
    protected override IAction CountDownAction(float remainTime)
    {
        ...
        {
            if (AirAnchor.CanUse(out var act1))
            {
                return act1;
            }
            else if (!AirAnchor.EnoughLevel && HotShot.CanUse(out act1))
            {
                return act1;
            }
        }
        ...
        if (remainTime < 5 && Reassemble.CanUse(out act, CanUseOption.EmptyOrSkipCombo | CanUseOption.IgnoreClippingCheck))
        {
            return act;
        }
    ...
    {
        //Overheated
        if (AutoCrossbow.CanUse(out act))
        {
            return true;
        }

        if (HeatBlast.CanUse(out act))
        {
            return true;
        }

        //Long Cds
        if (BioBlaster.CanUse(out act))
        {
            return true;
        }
        ...
        {
            if (AirAnchor.CanUse(out act))
            {
                return true;
            }
            else if (!AirAnchor.EnoughLevel && HotShot.CanUse(out act))
            {
                return true;
            }
        ...
        if (!CombatElapsedLessGCD(4) && ChainSaw.CanUse(out act, CanUseOption.MustUse))
        {
            return true;
        }

        //Aoe
        if (ChainSaw.CanUse(out act))
        {
            return true;
        }

        if (SpreadShot.CanUse(out act))
        {
            return true;
        }

        //Single
        if (CleanShot.CanUse(out act))
        {
            return true;
        }

        if (SlugShot.CanUse(out act))
        {
            return true;
        }

        if (SplitShot.CanUse(out act))
        {
            return true;
        }
    ...
    protected override bool EmergencyAbility(IAction nextGCD, out IAction act)
    {
        ...
        {
            if (Reassemble.CanUse(out act, CanUseOption.EmptyOrSkipCombo))
            {
                return true;
            }
        }
        if (Ricochet.CanUse(out act, CanUseOption.MustUse))
        {
            return true;
        }

        if (GaussRound.CanUse(out act, CanUseOption.MustUse))
        {
            return true;
        }
    ...
    protected override bool AttackAbility(out IAction act)
    {
        ...
        if (!CombatElapsedLess(12) && CanUseHypercharge(out act))
        {
            return true;
        }

        if (CanUseRookAutoturret(out act))
        {
            return true;
        }

        if (BarrelStabilizer.CanUse(out act))
        {
            return true;
        }
        ...
        {
            if (Ricochet.CanUse(out act, option))
            {
                return true;
            }
        }
        if (GaussRound.CanUse(out act, option))
        {
            return true;
        }
    ...
    private bool CanUseRookAutoturret(out IAction act)
    {
        ...
        if (AirAnchor.EnoughLevel)
        {
            if (!AirAnchor.IsCoolingDown || AirAnchor.ElapsedAfter(18))
            {
                return false;
            }
        }
        ...
        {
            if (!HotShot.IsCoolingDown || HotShot.ElapsedAfter(18))
            {
                return false;
            }
        }
    ...
    private static bool CanUseHypercharge(out IAction act)
    {
        ...
        if (!SpreadShot.CanUse(out _))
        {
            if (AirAnchor.EnoughLevel)
            {
                if (AirAnchor.WillHaveOneCharge(REST_TIME))
                {
                    return false;
                }
            }
            ...
            {
                if (HotShot.EnoughLevel && HotShot.WillHaveOneCharge(REST_TIME))
                {
                    return false;
                }
            }
        }
        if (Drill.EnoughLevel && Drill.WillHaveOneCharge(REST_TIME))
        {
            return false;
        }

        if (ChainSaw.EnoughLevel && ChainSaw.WillHaveOneCharge(REST_TIME))
        {
            return false;
        }
